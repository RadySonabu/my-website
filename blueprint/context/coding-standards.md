...
> Your conventions, tuned by `/onboard` to the detected stack: Next.js 16 (App
> Router) + React 19 + TypeScript + Tailwind v4, no `src/` directory, npm as the
> package manager. No database, auth, or ORM is wired up yet; the Database and
> Data Fetching sections below are TODOs until one is chosen.
...
> TODO: no database is configured yet. Pick a stack (e.g. Prisma + Postgres) when
> the project needs persistence, and update this section then.
